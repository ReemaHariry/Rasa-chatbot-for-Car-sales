...
A simple AI chatbot built using Rasa for handling car sales conversations — including greeting users, showing available car models, answering FAQs, checking prices, providing car features/specs, and scheduling test drives.
...
- `data/nlu.yml` – Intent examples for training the NLU, including robust examples for car makes, models, years, and slot-filling
- `data/rules.yml` and `data/stories.yml` – Define the dialogue flows, including slot-filling for car features
- `domain.yml` – Contains intents, responses, entities, slots, and actions (with correct slot/entity names: `car_make`, `car_model`, `year`)
- `actions/` – Folder for custom Python actions (each action in its own file)
- `models/` – Stores trained models
- `config.yml` – ML pipeline and policy configuration
...
Use this command anytime you modify intents, entities, slots, or stories:
```bash
rasa train
```
### ✅ 7. To have a conversation with the bot in cmd
```bash
rasa shell
```
to exit the conversation `CTRL+C`
...
- **Slots:**
  ...
  - `year`
- **Responses:**
  - Added `utter_ask_car_make`, `utter_ask_car_model`, `utter_ask_year` for slot-filling.
- **Actions:**
  - `action_show_available_models` 
  - `action_schedule_test_drive`
  - `action_provide_car_features` 


### 📄 stories.yml
- Added stories for all slot-filling scenarios:
  - User provides all info (make, model, year)
  - User provides only some info (bot asks for missing info)
  - User provides info in follow-up turns
